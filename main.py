...
# --- KUZATUVCHI (Mijozni xabardor qilish) ---
        ...
        try:
            ...
            if orders:
                for order_id, data in orders.items():
                    # Faqat haydovchi qabul qilgan va hali xabar bormagan bo'lsa
...
async def cmd_start(message: Message):
    ...
    if user_data and user_data.get("phone"): # Faqat raqami borlar kirsin
...
async def handle_location(message: Message):
    ...
    await message.answer("🚕 Buyurtma yuborildi. Haydovchi kutilmoqda...", reply_markup=kb_client)
    ...
    for d_id, d_data in all_users.items():
        if d_data.get("role") == "driver":
            # driver_id ni ham linkka qo'shdik!
